...
deconv_norm_type <- 'deseq2_vst' # c('q3_norm', 'log_norm', 'deseq2', 'deseq2_vst', 'libsize_log') which norm should be used for bayesprism results
...
norm_is_log <- ifelse(deconv_norm_type %in%  c('q3_norm', 'deseq2'), FALSE, TRUE)
...
#     deconv_ct_cleaned_norm <- varianceStabilizingTransformation(round(deconv_ct_cleaned))
#     
...
#     qs <- apply(deconv_ct_cleaned, 2, function(x) stats::quantile(x, 0.75))
#     deconv_ct_cleaned_norm <- sweep(deconv_ct_cleaned, 2L, qs / ngeoMean(qs), FUN = "/")
#     
#     # try with log_norm but as bad as q3
#     #deconv_ct_cleaned_norm <- NormalizeData(deconv_ct_cleaned, normalization.method = "LogNormalize", scale.factor = 10000)
#     
...
# pull all deconv cells together and norm ---------------------------------
