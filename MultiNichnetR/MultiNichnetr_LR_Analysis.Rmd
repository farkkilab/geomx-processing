...
output_dir = "C:/Users/Sahas/Downloads/Masters_Thesis/Ligand-receptor/MultiNichnetR/poststromaVsposttumor_run_02/"

NACT_status_filtered_group = "post"
...
# Set contrasts
contrasts_oi <- c("'tumor-stroma','stroma-tumor'")

# Create a contrast table
contrast_tbl <- tibble(contrast = c("tumor-stroma","stroma-tumor"), 
                       group = c("tumor", "stroma"))  
```
...
deconv_ct_list <- setNames(lapply(ct_names, function(ct_name) {
  ...
  if(!is.null(cell_frac_cutoff)){
    ...
    deconv_ct <- deconv_ct[(rownames(deconv_ct) %in% cells_greater_than_cutoff), ]
...
saveRDS(deconv_ct_list_combined, file = paste0(output_dir,"deconv_ct_list_combined.RDS"))
...
saveRDS(metadt_all, file = paste0(output_dir,"metadt_all.RDS"))
...
saveRDS(sender_receiver_de, file = paste0(output_dir,"sender_receiver_de.RDS"))
...
saveRDS(ligand_activities_targets_DEgenes, file = paste0(output_dir,"ligand_activities_targets_DEgenes.RDS"))
```
...
saveRDS(prioritization_tables, file = paste0(output_dir,"prioritization_tables.RDS"))
